...
def pars_hh(url, city=None, language=None):
	jobs = []
	errors = []
	if url:
		...
		if resp.status_code == 200:
			...
			if div_lst:
				for div in div_lst:
					...
					jobs.append({'title': vacancy, 'url': href,
...
def pars_habr(url, city=None, language=None):
	jobs = []
	errors = []
		...
		if resp.status_code == 200:
			...
			if div_lst:
				for div in div_lst:
					...
					jobs.append({'title': vacancy, 'url': domain + href,
								'description': content, 'company': company,
								'city_id': city, 'language_id': language})
			else:
				errors.append({'url': url, 'title': 'Div does not exists'})
...
def pars_job(url, city=None, language=None):
	jobs = []
	errors = []
		...
		if resp.status_code == 200:
			...
			if div_lst:
				for div in div_lst:
					...
					if not div.find(attrs={'class': '_1G5lt _3EXZS _3pAka _3GChV _2GgYH'}):
						content = 'None'
					else:
